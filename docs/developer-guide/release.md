...
- nova 主工程
- installation 部署脚本工程
- document 文档项目（暂时为最新的版本文档，后续会支持多版本）
...
release installationation部署脚本工程，和上述1，2步骤相同
...
这里的测试会使用installationation部署脚本里相应的release分支，同时回归验证部署脚本正确性。  
...
git push v${A.B.C}
```
注意Nova工程和Nova-installationation部署脚本均需要。

#### 6. 在Github上发布release

- Nova工程复制项目中的CHANGELOG即可
- Nova installation工程需要提供`Nova-linux-amd64`二进制和`Nova-v${A.B.C}.tgz` helm chart包。
  - 二进制需要找台linux amd64机器进行`make build`
  - helm chart包需要基于对应版本的Nova installation项目，执行`helm package ./helm-chart`即可。

#### 7. 更新Nova docs文档

- 更新Nova docs文档，并提PR合入
- 注意修改Nova镜像和版本号等


#### 8. 将release分支合入main
提PR将release分支合入main，可等稳定一段时间后再合入。


## 有BugFix时

- 如果是不重要的bug，基于main分支提交修改即可，可以不合入release分支
- 如果为重要的bug，需要修复，确定需要修复的版本release分支（可为最近一个或者多个），除了提交至main分支外，还需要cherry-pick至指定release分支，同时打上tag，新增最小版本号，同时需要确认installation和docs是否同步修改。